...
class VimEditor extends CustomEditor {
  ...
  private pendingInsertRepeat: {
    linewise: boolean;
    lineCount: number;
    charLen: number;
    insertStartOffset: number;
    beforeInsertText: string;
  } | null = null;
  ...
  private handleNormal(key: string): void {
    ...
    switch (key) {
      ...
      case "g":
        ...
        return;
      case "G":
        this.gotoLine(this.takeCountOrLast());
        return;
      case "i":
        this.mode = "insert";
        this.clearCount();
      ...
      case "o":
        this.openLineBelow();
        this.mode = "insert";
        this.saveLastChange(() => this.openLineBelow());
        return;
      case "O":
        this.openLineAbove();
        this.mode = "insert";
        this.saveLastChange(() => this.openLineAbove());
        return;
      case "x":
        this.applyOperator("d", this.charRangeForward(this.takeCount()), false);
        return;
      case "X":
        this.applyOperator(
          "d",
          this.charRangeBackward(this.takeCount()),
          false,
        );
        return;
      case "s":
        this.applyOperator("c", this.charRangeForward(this.takeCount()), true);
        return;
      case "S":
        this.applyOperator(
          "c",
          this.lineRange(this.cursor(), this.takeCount()),
          true,
        );
        return;
      case "D":
        this.applyOperator("d", this.motionRange("$", 1), false);
        return;
      case "C":
        this.applyOperator("c", this.motionRange("$", 1), true);
        return;
      case "Y":
        this.applyOperator(
          "y",
      ...
      case "V":
        this.mode = "visual";
        this.visualStart = { line: this.cursor().line, col: 0 };
        this.visualLine = true;
        this.move("$", 1);
        this.clearCount();
        return;
      case "r":
        this.mode = "replace-pending";
        this.clearCount();
        return;
      case "d":
      case "c":
  ...
  private handleOperatorPending(key: string): void {
    if (this.pendingG) {
      this.pendingG = false;
      if (key === "g") {
        this.gotoLine(this.takeCountOrFirst());
        this.toNormal();
        return;
      }
      this.toNormal();
    ...
    if (this.operator && this.isMotion(key)) {
      const count =
        key === "G" && !this.count
          ? this.editorState().lines.length
          : this.takeCombinedCount();
      this.applyOperator(
        this.operator,
        this.motionRange(key, count),
        this.operator === "c",
      );
      return;
    }
  ...
  private finishTextObject(key: string): void {
    if (!this.operator || !this.pendingTextObjectKind) {
      this.toNormal();
      return;
    }
  ...
  private motionRange(motion: string, count: number): Range {
    ...
    if (motion === "G") this.gotoLine(count);
    else this.move(motion, count);
    ...
    return this.rangeBetween(
      ...
      motion === "j" || motion === "k",
      motion !== "h" && motion !== "b" && motion !== "B" && motion !== "$",
    );
  }
    ...
    for (let i = 0; i < Math.max(1, count); i++) {
      ...
      switch (motion) {
        case "h":
          ...
          break;
        case "l":
          col = Math.min(this.line(line).length, col + 1);
          break;
        case "j":
          line = Math.min(s.lines.length - 1, line + 1);
          col = Math.min(col, this.line(line).length);
          break;
        case "k":
        ...
        case "w":
        case "W":
          ({ line, col } = this.nextWord(
            this.offset({ line, col }),
            motion === "W",
          ));
          break;
        case "e":
        case "E":
          ({ line, col } = this.endWord(
            this.offset({ line, col }),
            motion === "E",
          ));
          break;
  ...
  private applyOperator(
    ...
    if (enterInsert) {
      ...
      this.pendingInsertRepeat = {
        ...
        charLen: Math.max(
          0,
          this.offset(normalized.end) - this.offset(normalized.start),
        ),
  ...
  private textObjectRange(kind: "i" | "a", object: string): Range | null {
    if (object === "w" || object === "W")
      return this.wordObject(kind === "a", object === "W");
    ...
    return {
      start: this.pos(left + (kind === "i" ? 1 : 0)),
      end: this.pos(right + (kind === "i" ? 0 : 1)),
      inclusive: false,
    };
  }
  ...
  private wordObject(around: boolean, big: boolean): Range {
    ...
    const same = (a?: string, b?: string) =>
      big ? !isWs(a) && !isWs(b) : isWord(a) && isWord(b);
  ...
  private endWord(off: number, big: boolean): Pos {
    ...
    while (
      i + 1 < text.length &&
      (big ? !isWs(text[i + 1]) : isWord(text[i + 1]))
    )
      i++;
    return this.pos(i);
  }
  ...
  private replaceChar(ch: string): void {
    ...
    const range = {
      start: pos,
      end: { line: pos.line, col: pos.col + 1 },
      inclusive: false,
    };
  ...
  private putLinewise(after: boolean): void {
    ...
    const text = this.text();
    const replacement =
      insertLine >= this.editorState().lines.length ? `\n${body}` : `${body}\n`;
    const pos =
      insertLine >= this.editorState().lines.length
    ...
    this.setCursor({
      line: Math.min(insertLine, this.editorState().lines.length - 1),
      col: 0,
    });
  }
  ...
  private lineRange(start: Pos, count: number): Range {
    const last = Math.min(
      this.editorState().lines.length - 1,
      start.line + Math.max(1, count) - 1,
    );
    return {
      start: { line: start.line, col: 0 },
      end: { line: last, col: this.line(last).length },
      linewise: true,
      inclusive: true,
    };
  }
  ...
  private rangeBetween(
    ...
    return this.offset(a) <= this.offset(b)
      ? { start: a, end: b, linewise, inclusive }
      : { start: b, end: a, linewise, inclusive };
  }
  ...
  private normalizeRange(range: Range): Range {
    ...
    return {
      ...
      end: this.pos(Math.min(this.text().length, endOff)),
      inclusive: false,
    };
  }
  ...
  private sliceRange(range: Range): string {
    if (range.linewise) {
      return (
        this.editorState()
          .lines.slice(range.start.line, range.end.line + 1)
          .join("\n") + "\n"
      );
    }
  ...
  private replaceRange(range: Range, replacement: string): void {
    ...
    if (range.linewise) {
      start = this.offset({ line: range.start.line, col: 0 });
        ...
        range.end.line + 1 < this.editorState().lines.length
          ? this.offset({ line: range.end.line + 1, col: 0 })
          : text.length;
    ...
    this.setCursor(
      range.linewise
        ...
        : this.pos(start + replacement.length),
    );
  }
  ...
  private saveRepeat(range: Range, insertedText: string): void {
    ...
    const charLen = Math.max(
      0,
      this.offset(range.end) - this.offset(range.start),
    );
    this.saveLastChange(() => {
      ...
      const repeatRange = linewise
        ...
        : {
            start: cur,
            end: this.pos(this.offset(cur) + charLen),
            inclusive: false,
          };
  ...
  private finalizeInsertRepeat(): void {
    ...
    if (
      ...
      cursorOff < pending.insertStartOffset
    )
      return;
    ...
    this.saveLastChange(() => {
      ...
      const repeatRange = pending.linewise
        ...
        : {
            start: cur,
            end: this.pos(this.offset(cur) + pending.charLen),
            inclusive: false,
          };
      this.register = this.sliceRange(repeatRange);
  ...
  private gotoLine(line: number): void {
    this.setCursor({
      line: clamp(line - 1, 0, this.editorState().lines.length - 1),
      col: 0,
    });
    this.clearCount();
  }
  private takeCount(): number {
    const n = this.count ? Number(this.count) : 1;
    this.clearCount();
    return n;
  }
  private takeCombinedCount(): number {
    const motion = this.count ? Number(this.count) : 1;
    this.clearCount();
    return this.opCount * motion;
  }
  private takeCountOrFirst(): number {
    const n = this.count ? Number(this.count) : 1;
    this.clearCount();
    return n;
  }
  private takeCountOrLast(): number {
    const n = this.count ? Number(this.count) : this.editorState().lines.length;
    this.clearCount();
    return n;
  }
  private acceptCount(key: string): boolean {
  ...
  private toNormal(): void {
    this.mode = "normal";
    this.operator = null;
    this.operatorStart = null;
    this.pendingG = false;
    this.pendingTextObjectKind = null;
    this.pendingInsertRepeat = null;
    this.count = "";
    this.opCount = 1;
  ...
  private saveLastChange(fn: () => void): void {
    this.lastChange = fn;
  }
  private pushUndoSnapshotCompat(): void {
  ...
  private line(line: number): string {
    return this.editorState().lines[line] ?? "";
  }
  private text(): string {
    return this.editorState().lines.join("\n");
  }
  private setCursor(pos: Pos): void {
    const s = this.editorState();
    s.cursorLine = clamp(pos.line, 0, s.lines.length - 1);
    s.cursorCol = clamp(pos.col, 0, this.line(s.cursorLine).length);
  }
  private offset(pos: Pos): number {
    let off = 0;
    for (let i = 0; i < pos.line; i++) off += this.line(i).length + 1;
    return off + pos.col;
  }
  private pos(offset: number): Pos {
    let rest = clamp(offset, 0, this.text().length);
  ...
  private passControlOrIgnore(data: string): void {
  ...
  render(width: number): string[] {
    ...
    const pending =
      this.mode === "operator-pending" && this.operator
        ? ` ${this.operator}`
        : this.count
          ? ` ${this.count}`
          : "";
