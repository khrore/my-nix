...
- Why it matters: Database, HTTP, RPC, queue, and long-running operations should accept and pass `context.Context` so
...
- Why it matters: Ignored errors, broad fallback behavior, or string-only errors hide failed I/O and make debugging
...
- Why it matters: Leaked `Rows`, HTTP response bodies, files, tickers, timers, and goroutines can exhaust resources under
  load.
- Possible responses:
...
- Why it matters: Goroutines started from handlers or workers must terminate when the request/job is canceled. Sends to
...
- Why it matters: HTTP/RPC handlers should decode, validate, authorize, call services, and encode responses. Business
...
- Why it matters: Shared maps, slices, caches, metrics state, and lazy initialization can race across requests or workers.
- Possible responses:
...
- Why it matters: Backend logic should cover success, validation, authorization, persistence failure, context cancellation,
  and boundary cases.
- Possible responses:
