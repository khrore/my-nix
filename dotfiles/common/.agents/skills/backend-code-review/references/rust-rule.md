...
# Review Cues — Rust Backend Review
...
- Why it matters: Backend code reachable from user input, external APIs, queues, files, or databases must not panic on
...
- Why it matters: Blocking filesystem, network, CPU-heavy, or synchronous database work inside an async runtime can stall
...
- Why it matters: Stringly typed errors or erased context make it hard for callers to distinguish validation, not-found,
...
- Why it matters: Unnecessary `clone()` calls can hide ownership problems and add avoidable allocation costs on hot paths.
- Possible responses:
...
- Why it matters: Spawned async tasks must own required data safely, finish on shutdown/cancellation, and not hold
...
- Why it matters: Holding a transaction across external calls or many `.await` points increases lock duration and deadlock
...
- Why it matters: `unsafe` blocks must be rare in backend application code and must document the invariants that make them
...
- Why it matters: Backend tests should cover success, validation, authorization, repository failures, cancellation/timeouts,
